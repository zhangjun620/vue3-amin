...
import { userInfosModule } from '/@/stores/userInfos.ts'

const { t } = useI18n();

const userInfosModules = userInfosModule()
...
const onSignIn = async () => {
	...
	// 1、请注意执行顺序(存储用户信息到vuex) store.state.themeConfig.themeConfig.isRequestRoutes
	
	if (true) {
		// 前端控制路由，2、请注意执行顺序
		// await getUserInfo(userInfos).then(res => {
		// 	console.log('get userinfo ', res);
		// 	userInfosModules.setUserInfos(res)
		// })
...
const signInSuccess = () => {
	...
	if (route.query?.redirect) {
		console.log('in signinSuccess if');
		router.push({
			path: <string>route.query?.redirect,
			query: Object.keys(<string>route.query?.params).length > 0 ? JSON.parse(<string>route.query?.params) : '',
		});
		console.log('handle after push');
		
	} else {
		console.log('in signinSuccess else ');
		router.push('/');
	}
	// 登录成功提示
	// 关闭 loading
